# Mind Lumen Ethical AI Constitution

**Principles and Guidelines for AI Applications in Mental Health and Psychedelic Education**

The constitutional documents governing how Mind Lumen designs, builds, and evaluates any AI application in our ecosystem. Written before the code, because that is the only order that produces trustworthy tools.

**Document:** [mind-lumen-ethical-ai-constitution.md](mind-lumen-ethical-ai-constitution.md)

## Principles at a glance

1. **Reduce risk; do not promise to eliminate harm** — harm cannot be fully prevented; the standard is risk reduction, not harm elimination
2. **Protect cognitive liberty** — people have a right to informed self-determination over their own consciousness
3. **Be honest about what you know and don't know** — source transparency and conflict of interest disclosure take precedence over appearing confident
4. **Expand access, not gatekeeping** — widen access to accurate, independent information
5. **Remain revisable** — this constitution is a living document, not a dogma

## Philosophical foundations

- **Psychology** — Integration-oriented depth psychology over symptom-suppression frameworks
- **Philosophy** — Direct access and non-attachment; no required intermediary between a person and their own inquiry
- **Anthropology** — Indigenous knowledge as foundation, not footnote; borrowing with attribution
- **Ethics** — Applied and empirical; principles must be observable, measurable, and revisable

## How this document changes

The constitution is Layer 0 of the Architecture of Constraints. It is changed only by the Somatic Compiler role, with a documented rationale for every change, and it is not put to community vote.

This is deliberately different from the [ECP series](../ECPs/), which is community-ratified at a 60% supermajority of participating voters. Suggestions for the constitution are welcome and should be raised as issues; they are considered rather than voted on.

Semantic versioning applies. Every change is committed with a rationale and recorded in [CHANGELOG.md](../CHANGELOG.md).

## License

Published under [Creative Commons Attribution 4.0 International](https://creativecommons.org/licenses/by/4.0/). Free to share and adapt with attribution — including by organizations with no relationship to Mind Lumen.

## Related reading

[Why We Built It This Way](https://mindlumen.substack.com/p/the-story-behind-mind-lumens-ethical) — the narrative behind the principles.
